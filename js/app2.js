...
*/								console.log(mimo[i]);
								var data = mimo.slice();
								var arcColor = function(d) { return d.color };
								var dataLabel = function(d) { return d.label };
								var dataValue = function(d) { return d.quantity };

								var map = d3.map(data, function(d) { return d.quantity });
								
								var color = d3.scale.ordinal()
									.range(d3.extent(data, arcColor));

								var slice = svg.select(".slices").selectAll("path.slice")
									.data(pie(mimo[i]), function(d) { return d.quantity });
								//console.log(d);
